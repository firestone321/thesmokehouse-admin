...
export async function getDailyCloseData(serviceDate: string): Promise<DailyCloseData> {
  ...
  return { serviceDate, totalSales: posSales + onlineSales, posSales, onlineSales, completedPaidSales: orders.filter((order) => order.paymentStatus === "paid" && order.status === "completed").reduce((total, order) => total + order.totalAmount, 0), completedOrders: orders.filter((order) => order.paymentStatus === "paid" && order.status === "completed").length, cancelledOrders: orders.filter((order) => order.status === "cancelled").length, pendingPaymentOrders: orders.filter((order) => order.status !== "cancelled" && order.paymentStatus !== "paid").length, openPaidOrders: eligiblePaid.filter((order) => order.status !== "completed").slice(0, 12), posTenders, expectedPosCash: posTenders.find((tender) => tender.type === "cash")?.amount ?? 0, checklist, snapshot: row ? { id: String(row.id), closedByEmail: String(row.closed_by_email_snapshot), closedByRole: String(row.closed_by_role_snapshot), closedAt: String(row.closed_at), openingFloat: number(row.opening_float_ugx), cashCounted: number(row.cash_counted_ugx), expectedPosCash: number(row.expected_pos_cash_ugx), cashDifference: number(row.cash_difference_ugx), notes: typeof row.notes === "string" ? row.notes : null } : null };
}