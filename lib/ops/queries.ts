...
export async function getMenuPageData(editMenuItemId?: string | null) {
  ...
  const menuItems: MenuItemRecord[] = (menuItemsResponse.data ?? []).map((item: any) => {
    ...
    return {
      ...
      availabilityDays: Array.isArray(item.availability_days) ? item.availability_days.map(normalizeNumber) : [0, 1, 2, 3, 4, 5, 6],
      availabilityStartDate: item.availability_start_date ?? null,
      availabilityEndDate: item.availability_end_date ?? null,
