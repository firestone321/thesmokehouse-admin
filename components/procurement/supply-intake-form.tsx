...
export function SupplyIntakeForm({
  ...
  const supplyItems = useMemo(
    () => inventoryItems.filter((item) => item.itemType === "supply"),
    [inventoryItems]
  );
  const [itemOptions, setItemOptions] = useState(supplyItems);
  const [selectedItemId, setSelectedItemId] = useState<string>(
    defaultInventoryItemId ? String(defaultInventoryItemId) : supplyItems[0] ? String(supplyItems[0].id) : ""
  );
  ...
  const [isQuickAddItemOpen, setIsQuickAddItemOpen] = useState(supplyItems.length === 0);
  ...
  useEffect(() => {
    ...
    setSelectedItemId((currentItemId) => currentItemId || (supplyItems[0] ? String(supplyItems[0].id) : ""));
  }, [supplyItems]);
  ...
  async function handleQuickAddSupplier(event: React.FormEvent<HTMLFormElement>) {
    ...
    startCreateSupplierTransition(async () => {
      try {
        ...
        setQuickAddSuccess(`${result.supplier.name} is ready to use for this resupply.`);
  ...
  async function handleQuickAddItem(event: React.FormEvent<HTMLFormElement>) {
    ...
    startCreateItemTransition(async () => {
      try {
        ...
        if (!result.ok) {
          setQuickAddItemError("Unable to create tracked item.");
          return;
        }
        ...
        setQuickAddItemSuccess(`${result.item.name} is ready to use for this resupply.`);
        setIsQuickAddItemOpen(false);
        form.reset();
      } catch (error) {
        setQuickAddItemError(error instanceof Error ? error.message : "Unable to create tracked item.");
      }
    });
  }
  ...
  return (
    <section className="surface-card rounded-[32px] p-5">
      ...
      >
        <p className="text-[11px] uppercase tracking-[0.18em] text-[#9CA3AF]">Non-Consumable Receiving</p>
        <div className="mt-2 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold">Receive packaging and tracked supplies</h2>
      ...
      {!isCollapsed ? (
              ...
              <div className="rounded-[24px] border border-[#E4E7EB] bg-[#F8FAFB] px-4 py-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-2 text-lg font-semibold text-[#111418]">Quick add a non-consumables supplier</h3>
                  </div>
                ...
                <div className="mt-4 grid gap-3">
                  <input
                    form="supply-quick-add-supplier-form"
                    name="name"
                    required
                  ...
                  />
                  <input form="supply-quick-add-supplier-form" type="hidden" name="supplier_type" value="supply" />
                  <label className="flex items-center gap-2 text-sm text-[#6B7280]">
                    <input form="supply-quick-add-supplier-form" type="checkbox" name="is_active" defaultChecked />
                    Active supplier
                  </label>
                  {quickAddError ? (
            ...
            {isQuickAddItemOpen ? (
              <div className="rounded-[24px] border border-[#E4E7EB] bg-[#F8FAFB] px-4 py-4">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-semibold text-[#111418]">Quick add a supply item</h3>
                </div>

                <div className="mt-4 grid gap-3">
                  <input
                    form="supply-quick-add-item-form"
                    name="name"
                    required
                    placeholder="Item name, e.g. paper cups"
                    className="rounded-2xl border border-[#D7DDE4] bg-white px-3 py-2.5 text-sm text-[#111418]"
                  />
                  <input
                    form="supply-quick-add-item-form"
                    name="code"
                    placeholder="Code, e.g. paper_cups"
                    className="rounded-2xl border border-[#D7DDE4] bg-white px-3 py-2.5 text-sm text-[#111418]"
                  />
                  <input
                    form="supply-quick-add-item-form"
                    name="unit_name"
                    required
                    placeholder="Unit, e.g. piece, pack, box"
                    className="rounded-2xl border border-[#D7DDE4] bg-white px-3 py-2.5 text-sm text-[#111418]"
                  />
                  <input
                    form="supply-quick-add-item-form"
                    ...
                    className="rounded-2xl border border-[#D7DDE4] bg-white px-3 py-2.5 text-sm text-[#111418]"
                  />
                  <input form="supply-quick-add-item-form" type="hidden" name="item_type" value="supply" />
                  {quickAddItemError ? (
                    <div className="rounded-[20px] border border-[#F4C7C7] bg-[#FFF8F8] px-4 py-3 text-sm leading-6 text-[#8A1C1C]">
                      {quickAddItemError}
                    </div>
        ...
        {supplierOptions.length === 0 ? (
          <p className="text-sm leading-6 text-[#6B7280]">
            Add a supplier first so this resupply is recorded against a saved non-consumables supplier.
          </p>
        ) : null}
        {itemOptions.length === 0 ? (
          <p className="text-sm leading-6 text-[#6B7280]">
            Add a tracked item first so this resupply can be recorded into inventory.
          </p>
        ) : null}
          </form>
          <form id="supply-quick-add-supplier-form" onSubmit={handleQuickAddSupplier}></form>
          <form id="supply-quick-add-item-form" onSubmit={handleQuickAddItem}></form>
        </>
      ) : (
        <div className="mt-4 rounded-[22px] bg-[#F8FAFB] px-4 py-4 text-sm leading-6 text-[#6B7280]">
          Click the card header when you need to receive packaging or tracked operational supplies.
        </div>
      )}
    </section>
  );
