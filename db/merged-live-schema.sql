...
-- ---------------------------------------------------------------------------
-- Phase 61: bound, row-locked and monotonic Pesapal payment transitions
-- ---------------------------------------------------------------------------
...
-- Phase 63: shared menu category ordering.
...
begin
  select * into v_small_fries
  from public.portion_types as pt
  where pt.code in ('fries', 'fries_250g') and pt.is_active = true
  ...
  set
    ...
    source_menu_item_id = coalesce(source_menu_item_id, (
      select mi.id from public.menu_items as mi
  ...
  update public.portion_types
  -- Phase 55 keeps reserved/sold display counts separate by menu portion.
  set stock_source_portion_type_id = v_small_fries.id,
      stock_source_units_per_serving = 2
  where code in ('fries_large_portion', 'large_fries')
    and id <> v_small_fries.id;

  select coalesce(sum(pr.quantity_received), 0) into v_legacy_receipt_quantity
  from public.procurement_receipts as pr
  where pr.inventory_item_id = v_fries_input.id
    and pr.intake_type = 'ingredient';

  for v_receipt in
    select pr.* from public.procurement_receipts as pr
    where pr.inventory_item_id = v_fries_input.id
      and pr.intake_type = 'ingredient'
      and pr.quantity_received > 0
      and not exists (
        select 1 from public.finished_stock_movements as fsm
  ...
  loop
    ...
    insert into public.finished_stock_movements (
      ...
      v_small_fries.id, 'adjustment', v_sellable_quantity, v_finished_stock.current_quantity, null,
  ...
  if v_fries_input.current_quantity = v_legacy_receipt_quantity
    ...
    and not exists (
      select 1 from public.inventory_movements as im
    ...
    update public.inventory_items set current_quantity = 0 where id = v_fries_input.id;

    insert into public.inventory_movements (
      inventory_item_id, movement_type, quantity_delta, resulting_quantity, note
    ) values (
      v_fries_input.id, 'usage', -v_legacy_receipt_quantity, 0,
...
-- ---------------------------------------------------------------------------
-- Phase 65: staff activity and menu price approvals
-- ---------------------------------------------------------------------------
begin;
