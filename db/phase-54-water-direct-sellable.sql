...
-- Phase 54: water 500ml direct-sellable intake.
-- Purpose:
-- 1. Add water_carton inventory item (ingredient, tracked in cartons of 12 x 500ml bottles).
-- 2. Add water_500ml portion type and menu item in the drinks category.
-- 3. Update record_procurement_receipt so receiving one carton credits 12 x water_500ml
--    into sellable finished stock, matching the soda pattern.
...
values
  ('water_carton', 'Water cartons', 'carton', 'ingredient', 0, 0, true)
...
declare
  ...
  v_water_portion_id   bigint;
begin
  select id into v_drinks_category_id
  ...
  insert into public.portion_types (
    code, protein_id, packaging_type_id, name, portion_label, sort_order, is_active
  )
  values ('water_500ml', null, null, 'Water', '500ml', 12, true)
  on conflict (code) do update
  set
    name          = excluded.name,
  ...
  select id into v_water_portion_id
  from public.portion_types
  where code = 'water_500ml';

  insert into public.menu_items (
    code, menu_category_id, portion_type_id, name, description,
    base_price, prep_type, is_active, is_available_today, sort_order
  )
  select
    'water_500ml', v_drinks_category_id, v_water_portion_id, 'Water 500ml',
    'Chilled water bottle.', 0, 'drink', false, false, 12
  where v_water_portion_id is not null
    and not exists (
      select 1 from public.menu_items where code = 'water_500ml'
    );

  update public.menu_items
  set
    menu_category_id = v_drinks_category_id,
    portion_type_id  = v_water_portion_id,
    name             = 'Water 500ml',
...
begin
  ...
  if p_intake_type = 'protein' then
    ...
    if p_protein_code not in (
      'beef_ribs', 'beef_chunks', 'whole_chicken', 'goat_ribs', 'goat_chunks', 'beef', 'goat'
    ) then
    ...
    if p_intake_type = 'ingredient' then
      ...
      elsif v_inventory_item.code = 'water_carton' then
        if trunc(p_quantity_received) <> p_quantity_received then
          raise exception 'Water cartons must be received as whole cartons';
        end if;

        v_direct_sellable_portion_code := 'water_500ml';
        v_direct_sellable_quantity := (p_quantity_received::integer * 12);
      end if;
      ...
      if v_direct_sellable_portion_code is not null then
        ...
        if v_direct_sellable_portion_id is null then
          raise exception 'Direct sellable portion % is missing or inactive',
            v_direct_sellable_portion_code;
        end if;
      end if;
    end if;
...
revoke all on function public.record_procurement_receipt(
...
grant execute on function public.record_procurement_receipt(
  text, text, bigint, bigint, text, text, date, date, text, text, text, numeric, text, numeric, text, integer, integer
) to service_role;
...
comment on function public.record_procurement_receipt(
  ...
  'Records protein, ingredient, or supply procurement receipts. Fries, juice, yoghurt, soda, and water ingredient receipts credit sellable finished stock directly.';
