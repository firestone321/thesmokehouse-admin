...
-- Phase 55: Shared stock display + business-truth reconciliation.
-- Two RPC replacements that both rely on Phase 35's stock-source mapping:
--
-- 1. get_daily_menu_stock now derives reserved/sold counts per portion from
--    order_item_totals for every active portion, not just sourced ones. Effect:
--    when a customer orders large_fries, those sales no longer appear on the
--    fries_250g card. starting/remaining still use the shared physical pool, so
--    the fries_250g card correctly reflects the units actually still available.
--
-- 2. get_business_truth_health_snapshot's daily_paid_totals CTE is now
--    source-aware: order_items are grouped by their effective portion type and
--    multiplied by stock_source_units_per_serving, mirroring how Phase 35's
--    finalize_reserved_order_sale increments daily_stock.sold_quantity. Resolves
--    the false "Daily stock differs from paid order totals" critical that fired
--    whenever shared-stock portions (e.g. large_fries) were sold.
