...
-- Phase 25: processing posts same-day sellable stock.
-- Purpose:
-- 1. Keep the procurement processing workflow as the source of truth for finished protein output.
-- 2. Make newly processed portions visible in today's inventory/day-stock cards immediately.
-- 3. Preserve the existing finished_stock, finished_stock_movements, and processing_batches audit trail.
...
comment on function public.process_procurement_receipt_to_finished_stock(
  ...
  'Processes beef and goat protein receipts into finished frozen stock, records the finished-stock audit trail, and posts produced portions into today''s sellable daily stock.';
...
comment on function public.process_whole_chicken_receipt_allocation(
  ...
  'Processes a whole chicken receipt in one allocation event, splitting birds between halves and quarters while posting output to finished stock and today''s sellable daily stock.';
