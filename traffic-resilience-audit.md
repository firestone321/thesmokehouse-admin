...
| SH-P2-14 | Medium | optimized locally; live prerequisite readback passed; Phase 59 apply, storefront deploy, and live timing validation pending | `thesmokehouse-admin/db/phase-59-storefront-checkout-hot-path.sql`; `thesmokehouse/app/api/orders/route.ts`; `thesmokehouse/lib/payments/order-payments.ts`; `thesmokehouse/lib/payments/pesapal.ts` | A successful checkout serializes many Supabase requests around Pesapal, amplifying cold-instance latency | Customers wait longer for the Pesapal redirect; simultaneous cold checkouts can duplicate token/IPN setup requests | Apply Phase 59 before deploying the storefront, then compare `storefront_checkout_timing` and `pesapal_api_request_completed` logs across cold/warm checkouts while validating rejection, retry, delayed-IPN, duplicate-IPN, callback, and recovery paths | P1 |
...
| `/api/orders` | durable reservation lookup, combined rate-limit RPC, collapsed menu/stock RPC, atomic order/items/payment-attempt preparation, Pesapal, atomic payment/recovery/reservation finalization | 32KB, 50 lines, qty max 20, DB rate limit, request-hash idempotency; Phase 59 is local-only until applied | DEGRADED until Phase 59 deployment and live cold/warm timing validation; paid-stock contention remains separately mitigated |
